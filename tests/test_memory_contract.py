...
def test_governance_patch_marks_imported_memories_for_review():
    patch = build_memory_governance_patch(
        metadata={
            "type": "project_checkpoint",
            "project": "example-project",
            "labels": ["checkpoint"],
        },
        origin_collection="memories",
    )

    assert patch["knowledge_domain"] == "project_memory"
    assert patch["knowledge_role"] == "checkpoint"
    assert patch["trust_level"] == "needs_review"
    assert patch["retrieval_tier"] == "reviewed_only_when_relevant"
    assert patch["lifecycle_state"] == "imported"
    assert patch["evaluation_status"] == "pending_review"
    assert patch["evaluation_sequence"] == [
        "origin_preserved",
        "text_hash_checked",
        "domain_tagged",
...
def test_default_indexes_include_governance_fields():
    indexes = default_memory_indexes()
    for field in (
        "metadata.knowledge_domain",
        "metadata.knowledge_role",
        "metadata.trust_level",
        "metadata.retrieval_tier",
        "metadata.lifecycle_state",
        "metadata.evaluation_status",
        "metadata.evaluation_sequence",
        "metadata.review_status",
    ):
        assert field in indexes


def test_build_memory_filter_governance_fields():
    warnings: list[str] = []
    filt = build_memory_filter(
        {
            "knowledge_domain": "project_memory",
            "trust_level": "needs_review",
            "evaluation_status": "pending_review",
            "evaluation_sequence": "domain_tagged",
    ...
    assert filt is not None
    keys = {condition.key for condition in filt.must}
    assert "metadata.knowledge_domain" in keys
    assert "metadata.trust_level" in keys
    assert "metadata.evaluation_status" in keys
    assert "metadata.evaluation_sequence" in keys
    assert "metadata.review_status" in keys
