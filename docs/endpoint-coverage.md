...
| Collection info | `GET /collections/{collection_name}` | `qdrant-collection-info`, `qdrant-collection-stats`, `qdrant-collection-vectors`, `qdrant-collection-payload-schema` | Read-only | Unit/integration tests |
...
| Points search/query | query/search endpoints | `qdrant-find`, `qdrant-find-short-term`, `qdrant-recommend-memories` | Read-only external call | Unit/integration tests |
| Points scroll | `POST /collections/{collection_name}/points/scroll` | `qdrant-list-points`, maintenance scans | Read-only | Unit/integration tests |
