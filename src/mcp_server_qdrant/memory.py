...
class MemoryInput(BaseModel):
    ...
    labels: list[str] | None = Field(default=None, description="Optional labels for the memory.")
    ...
    expires_at: Any | None = Field(default=None, description="Optional expiration timestamp.")
...
class MemoryFilterInput(BaseModel):
    ...
    class_: str | None = Field(
        default=None,
        alias="class",
        description="Filter by exact school course or class code, such as COURSE101.",
    )
    ...
    module: str | int | None = Field(default=None, description="Filter by module number or label.")
    ...
    chapter_title: str | None = Field(default=None, description="Filter by exact chapter title.")
    ...
    section_heading: str | None = Field(default=None, description="Filter by section heading.")
    created_at_from: Any | None = Field(
        default=None, description="Filter by creation time (start)."
    )
    created_at_to: Any | None = Field(default=None, description="Filter by creation time (end).")
    updated_at_from: Any | None = Field(default=None, description="Filter by update time (start).")
    updated_at_to: Any | None = Field(default=None, description="Filter by update time (end).")
    expires_at_from: Any | None = Field(
        default=None, description="Filter by expiration time (start)."
    )
    expires_at_to: Any | None = Field(default=None, description="Filter by expiration time (end).")
    confidence_min: float | None = Field(default=None, description="Minimum confidence score.")
    confidence_max: float | None = Field(default=None, description="Maximum confidence score.")
    text_hash: str | None = Field(default=None, description="Filter by text hash.")
    knowledge_domain: str | None = Field(
        default=None, description="Filter by governance knowledge domain."
    )
    knowledge_role: str | None = Field(
        default=None, description="Filter by governance knowledge role."
    )
    trust_level: str | None = Field(default=None, description="Filter by governance trust level.")
    retrieval_tier: str | None = Field(
        default=None, description="Filter by governance retrieval tier."
    )
    lifecycle_state: str | None = Field(
        default=None, description="Filter by governance lifecycle state."
    )
    evaluation_status: str | None = Field(
        default=None, description="Filter by governance evaluation status."
    )
    evaluation_sequence: str | None = Field(
        default=None, description="Filter by governance evaluation sequence step."
    )
    review_status: str | None = Field(
        default=None, description="Filter by governance review status."
    )
    origin_collection: str | None = Field(
        default=None, description="Filter by original source collection."
    )
...
def _append_unique_label(labels: list[str], value: str) -> None:
    if value not in labels:
        labels.append(value)


def infer_knowledge_domain(metadata: dict[str, Any]) -> str:
    if metadata.get("client") or metadata.get("client_slug"):
        return "client_memory"
    if metadata.get("project"):
        return "project_memory"
    if metadata.get("class") or metadata.get("course") or metadata.get("subject"):
        return "academic_memory"
    return "general_memory"


def infer_knowledge_role(metadata: dict[str, Any]) -> str:
    memory_type = str(metadata.get("type") or "").lower()
    if "checkpoint" in memory_type:
        return "checkpoint"
    if any(token in memory_type for token in ("progress", "planning", "update")):
        return "operational_update"
    if "client" in memory_type:
        return "client_context"
    if metadata.get("doc_id") or metadata.get("doc_title"):
        return "document_context"
    return "context_memory"


def build_memory_governance_patch(
    *,
    metadata: dict[str, Any] | None,
    origin_collection: str | None = None,
) -> dict[str, Any]:
    """Return missing governance metadata without overwriting existing choices."""
    raw = dict(metadata or {})
    patch: dict[str, Any] = {}

    defaults = {
        "knowledge_domain": infer_knowledge_domain(raw),
        "knowledge_role": infer_knowledge_role(raw),
        "trust_level": "needs_review",
        "retrieval_tier": "reviewed_only_when_relevant",
        "lifecycle_state": "imported" if origin_collection else "active",
    ...
    sequence = raw.get("evaluation_sequence")
    if not isinstance(sequence, list) or not sequence:
        patch["evaluation_sequence"] = [
            "origin_preserved",
            "text_hash_checked",
            "domain_tagged",
            "trust_marked_needs_review",
        ]

    labels = raw.get("labels")
    if isinstance(labels, list):
        merged_labels = [str(label) for label in labels]
    else:
        tags = raw.get("tags")
        merged_labels = [str(tag) for tag in tags] if isinstance(tags, list) else []
    if origin_collection:
        _append_unique_label(merged_labels, f"imported-from-{origin_collection}")
    if merged_labels or origin_collection:
        _append_unique_label(merged_labels, "needs-review")
        patch["labels"] = merged_labels
...
def _coerce_float(value: Any, field_name: str, strict: bool, warnings: list[str]) -> float | None:
    if value is None:
        return None
...
def normalize_memory_input(
    *,
    ...
    if entities is None:
        ...
        entities_list = [_coerce_text(item, "entities", strict, warnings) for item in entities]
    else:
        if strict:
    ...
    if labels is not None:
        if isinstance(labels, str):
            ...
            labels_list = [_coerce_text(item, "labels", strict, warnings) for item in labels]
        else:
            if strict:
    ...
    if related_ids is not None:
        if isinstance(related_ids, str):
            related_ids_list = [item.strip() for item in related_ids.split(",") if item.strip()]
            warnings.append("related_ids coerced to list.")
        elif isinstance(related_ids, list):
            related_ids_list = [
                _coerce_text(item, "related_ids", strict, warnings) for item in related_ids
            ]
        else:
            if strict:
                raise ValueError("related_ids must be a list or string.")
            warnings.append("related_ids coerced to list.")
            related_ids_list = [_coerce_text(related_ids, "related_ids", strict, warnings)]
    ...
    confidence_value = _coerce_float(raw.get("confidence"), "confidence", strict, warnings)
    ...
    last_seen_at = _parse_datetime(raw.get("last_seen_at"), "last_seen_at", strict, warnings)
    ...
    chunks = chunk_text(text, max_text_length) if len(text) > max_text_length else [text]
...
def build_memory_filter(
    ...
    add_match("knowledge_domain", raw.get("knowledge_domain"))
    add_match("knowledge_role", raw.get("knowledge_role"))
    add_match("trust_level", raw.get("trust_level"))
    add_match("retrieval_tier", raw.get("retrieval_tier"))
    add_match("lifecycle_state", raw.get("lifecycle_state"))
    add_match("evaluation_status", raw.get("evaluation_status"))
    add_match("evaluation_sequence", raw.get("evaluation_sequence"))
    add_match("review_status", raw.get("review_status"))
    add_match("origin_collection", raw.get("origin_collection"))
    ...
    if related_ids is not None:
        if isinstance(related_ids, str):
            related_ids = [item.strip() for item in related_ids.split(",") if item.strip()]
...
def default_memory_indexes() -> dict[str, models.PayloadSchemaType]:
    return {
        ...
        f"{METADATA_PATH}.knowledge_domain": models.PayloadSchemaType.KEYWORD,
        f"{METADATA_PATH}.knowledge_role": models.PayloadSchemaType.KEYWORD,
        f"{METADATA_PATH}.trust_level": models.PayloadSchemaType.KEYWORD,
        f"{METADATA_PATH}.retrieval_tier": models.PayloadSchemaType.KEYWORD,
        f"{METADATA_PATH}.lifecycle_state": models.PayloadSchemaType.KEYWORD,
        f"{METADATA_PATH}.evaluation_status": models.PayloadSchemaType.KEYWORD,
        f"{METADATA_PATH}.evaluation_sequence": models.PayloadSchemaType.KEYWORD,
        f"{METADATA_PATH}.review_status": models.PayloadSchemaType.KEYWORD,
        f"{METADATA_PATH}.origin_collection": models.PayloadSchemaType.KEYWORD,
    }
...
def build_memory_backfill_patch(
    *,
    ...
    strict: bool,
    include_governance_metadata: bool = False,
    origin_collection: str | None = None,
    ...
    if "type" not in metadata or not isinstance(metadata.get("type"), str):
        patch["type"] = (
            metadata.get("type") if isinstance(metadata.get("type"), str) else DEFAULT_MEMORY_TYPE
        )
    ...
    if "source" not in metadata or not isinstance(metadata.get("source"), str):
        patch["source"] = (
            metadata.get("source") if isinstance(metadata.get("source"), str) else DEFAULT_SOURCE
        )

    if "scope" not in metadata or not isinstance(metadata.get("scope"), str):
        patch["scope"] = (
            metadata.get("scope") if isinstance(metadata.get("scope"), str) else DEFAULT_SCOPE
        )
